...
This application does **not** ship a local `mcp_server/` component. Prediction and other
DataRobot platform tools are accessed through the **DataRobot Global MCP** service.

Configure in `.env` (see [Connect agentic coding environments to MCP servers](https://docs.datarobot.com/en/docs/agentic-ai/agentic-mcp/agentic-mcp-clients.html)):
...
Do **not** set `MCP_DEPLOYMENT_ID` — that would target a standalone deployed MCP server instead.

Domain-specific LangChain tools (`get_dam_prices`, `predict_dam_prices`,
`compute_accuracy_metrics`, etc.) live in `agent/agent/ercot_tools.py`. ERCOT DAM
**forecasts** must use `predict_dam_prices` (time-series batch scoring with
`ERCOT_DEPLOYMENT_ID`); Global MCP generic prediction tools are not suitable for
this deployment. Other Global MCP tools remain available via `myagent.py`.
