...
# ERCOT Forecast Agent App runtime parameters.
fastapi_server_app_runtime_parameters.append(
    pulumi_datarobot.ApplicationSourceRuntimeParameterValueArgs(
        type="string",
        key="ERCOT_DEPLOYMENT_ID",
        value=os.getenv("ERCOT_DEPLOYMENT_ID", ""),
    )
)
fastapi_server_app_runtime_parameters.append(
    pulumi_datarobot.ApplicationSourceRuntimeParameterValueArgs(
        type="string",
        key="ERCOT_DATASET_ID",
        value=os.getenv("ERCOT_DATASET_ID", "698dfe27b04f8da88246bc28"),
    )
)
if tavily_api_key := os.getenv("TAVILY_API_KEY"):
    tavily_cred = pulumi_datarobot.ApiTokenCredential(
        f"ERCOT Tavily API Key [{PROJECT_NAME}]",
